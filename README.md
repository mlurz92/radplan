...
*   **Wert-Interpretation (`TTI` in `analytics/engine.js`):** Über die reine Begriffsdefinition hinaus erklären die Tooltips an den **angezeigten Werten** das *konkrete Ergebnis* — sie lesen die jeweilige Zahl fachlich ein („was bedeutet die 82 hier?"). Eine zentrale Interpreter-Bibliothek liefert dafür einordnende Sätze inklusive qualitativer Bänder (sehr gut / gut / mittel / schwach / kritisch), z. B.: ein Equity-Index von 82/100 → „gut: überwiegend ausgewogene Verteilung mit leichten Unterschieden"; eine Fair-Δ von +3 → „leistet 3 Dienste mehr als den fairen Anteil – überdurchschnittlich belastet"; ein Risiko-Index, eine Abdeckungsquote, ein Compliance-Score, eine Prognose-Abweichung, eine Wunscherfüllungsrate oder ein Abwesenheits-Spitzentag werden jeweils mit Blick auf genau den dargestellten Wert interpretiert. So wird neben dem *Was* auch das *Was heißt das* unmittelbar verständlich.
