...
| `printpreview.js` | Druckvorschau-Dialog mit Layout-Optionen (Quer-/Hochformat), Ein-/Ausschluss der RD-Neurorad-Zeile, maßstabsgetreuer Seitenvorschau und Seitenumbruch-Hinweis. Bietet zwei Ausgabewege: nativer Browser-Druck (mit dynamisch gesetztem `@page`-Format) sowie native PDF-Generierung via jsPDF/AutoTable. Der **PDF-Export passt garantiert vollständig auf eine A4-Seite** (Querformat): Spaltenbreiten füllen exakt die nutzbare Seitenbreite, und eine iterative Auto-Fit-Schleife wählt die größte Schriftgröße, bei der die gesamte Tabelle vertikal auf eine Seite passt (Zellinhalte brechen um statt abzuschneiden). **Alle farblichen Grid-Markierungen werden übernommen** — Arbeitsplatz-/Status-Chipfarben (helle, themenunabhängige Druckpalette), Wochenende, Feiertag, Heute-Spalte, D-/HG-Dienst-Tints, Konflikt-Rahmen und die RD-Neurorad-Zeile — zuzüglich Kopfzeile mit eingebettetem App-Logo und Seitenzahl. |
