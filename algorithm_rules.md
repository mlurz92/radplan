...
Der Algorithmus läuft künstlich für exakt ~22 Sekunden in der **"Solver Sequencer"**-Visualisierung. Sie zeigt direkt die **Entscheidungsmatrix** des Planers im Stil eines Audio-/DAW-Sequencers: Zeilen sind die Mitarbeitenden (Fachärzte oben/eingefärbt, Assistenzärzte darunter), Spalten die Tage des Monats. Während der Lauf streamt, **füllt sich der Dienstplan live** – jede Vergabe setzt eine leuchtende Kachel (D rot, HG blau) an der Kreuzung Mitarbeiter|Tag. Ein **Playhead** fegt wie ein Abspielkopf über die Tage und lässt die Dienste der Spalte aufblitzen; **Swaps** springen sichtbar als Token zwischen den Mitarbeiter-Zeilen; rechts macht ein **Load-Meter** je Zeile die Dienstlast und damit die Fairness als Balkenbalance unmittelbar ablesbar. Die Phasenfarbe (Init/Greedy/HG/Deep/Erfolg) tönt Playhead und Glow, Fehler blitzen rot, der Erfolg läuft als grüne Welle über die Matrix. Das HUD oben rechts zeigt eine schnelle, digitale Telemetrie (Energie-/Kühlkurve, Spektrum-Equalizer, Hex-Ticker, blinkendes ΔE-Readout). So wird sichergestellt, dass die Rechentiefe ausgeschöpft wurde und dem Anwender das Volumen der simulierten Kombinationen eindrucksvoll veranschaulicht wird.
