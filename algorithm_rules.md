...
- **Gestaffelte BD-Ziele zur Einarbeitung (`SPECIAL_RULES.bdTargetSchedule`):** Für Neuzugänge kann das BD-Monatsziel zeitlich gestaffelt werden; die Staffel hat Vorrang vor `reducedBdTarget`/`maxBdTarget`/`minBdTarget` und läuft nach ihrer letzten Stufe wieder auf die statischen Regeln aus. **Hr. Safari** erhält damit im **1. Monat (November 2026) 0 Dienste** (harte Obergrenze 0, reine Einarbeitung), im **2. Monat (Dezember 2026) 3 Dienste** und **ab dem 3. Monat (Januar 2027) das reguläre AA-Standardziel von 4**. Die Getter `getReducedBdTarget`/`getMaxBdTarget`/`getMinBdTarget` sind dafür monatsabhängig und **müssen immer mit dem Planmonat `(y, m)` aufgerufen werden** — ein einarmiger Aufruf fällt still auf die statischen Regeln zurück und wird deshalb durch `test/bd-target-context.test.js` verhindert.
