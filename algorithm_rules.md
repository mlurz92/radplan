...
Der Algorithmus läuft künstlich für exakt ~22 Sekunden in der **"Annealing Field"**-Visualisierung. Sie bildet den Planer als das ab, was er mathematisch ist – einen iterativen Optimierer – und ist konzeptuell an **Simulated Annealing** angelehnt: Der gesamte Monat ist ein **dezentrales Energiefeld** (jeder Tag eine Zelle mit eigener Temperatur), **ohne zentralen Konvergenzpunkt**. Früh flackert das Feld heiß und chaotisch, mit jeder Phase (Init/Greedy/HG/Deep/Erfolg) kühlt es ab und kristallisiert. Vergaben und Swaps zünden lokal an der eigenen Zelle und pflanzen sich als farbcodierte Constraint-Wellen (D rot, HG blau) zu den Nachbartagen fort; ein schneller Optimierungs-Scan streicht quer über das ganze Feld. Das HUD oben rechts zeigt eine schnelle, digitale Telemetrie (scrollende Energie-Abstiegskurve, Spektrum-Equalizer, Hex-Ticker, blinkendes ΔE-Readout). So wird sichergestellt, dass die Rechentiefe ausgeschöpft wurde und dem Anwender das Volumen der simulierten Kombinationen eindrucksvoll veranschaulicht wird.
