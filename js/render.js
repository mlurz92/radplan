...
export function openProfileModal(empName) {
  ...
  if (wpChartEl) {
    ...
    if (wpEntries.length) {
      ...
      wpChartEl.innerHTML = wpEntries.map(([code, cnt]) => {
        ...
        const pct = s.totalWP > 0 ? Math.round((cnt/s.totalWP)*100) : 0;
        return `
          <div class="dist-row">
  ...
  if (yrEl) {
    ...
    ys.months.forEach(mon => {
      ...
      const apSum = Object.values(mon.wpCounts).reduce((a, b) => a + b, 0);
      
      yrHtml += `
        <tr class="yr-row${isCur ? " yr-row-current" : ""}${rc}">
          <td class="yr-td-month">${MONTHS_SHORT[mon.m]}</td>
          <td class="yr-td yr-td-num">${mon.hasData && mon.totalWorkdays > 0 ? (apSum || "—") : "—"}</td>
    ...
    yrHtml += `
          <tr class="yr-total-row">
            ...
            <td class="yr-td yr-td-num yr-total">${ys.totals.totalWP || "—"}</td>
  ...
  empStats.forEach(({ emp, s, meta, pc, vac, sick, fza, frei }) => {
    rowsHtml += `
      <tr class="dept-tr">
        ...
        <td class="dept-td dept-td-num">${s.totalWP || "—"}</td>
  ...
  const stripHtml = `
    <div class="dept-yr-strip">
      ...
      <div class="dept-yr-kpi">
        ...
        <span class="dept-yr-kpi-lbl">AP-Tage</span>
      </div>
      <div class="dept-yr-kpi">
  ...
  empYS.forEach(({ emp, ys, meta }) => {
    ...
    rowsHtml += `
      <tr class="dept-tr">
        ...
        <td class="dept-td dept-td-num">${t.totalWP || "—"}</td>
      ...
      return `
        <button type="button" class="empdash-card${selectedCls}" data-emp="${item.emp}" role="listitem">
          ...
          <div class="empdash-card-stats">
            <span><strong>${item.ys.totals.totalWP || 0}</strong><small>AP</small></span>
...
export function renderEmployeeDetailDashboard(emp, year) {
  ...
  if (state.employeeDashboard.detailView === 'months') {
    ...
    ys.months.forEach((mon) => {
      ...
      html += `
        <tr class="${isCur ? 'is-current' : ''}">
          ...
          <td>${Object.values(mon.wpCounts).reduce((a,b) => a + b, 0) || '—'}</td>
          <td>${vac || '—'}</td>
          <td>${sick || '—'}</td>
    ...
    const totalCov = ys.totals.totalWorkdays ? Math.round((ys.totals.coveredWorkdays / ys.totals.totalWorkdays) * 100) : 0;
  ...
  if (state.employeeDashboard.detailView === 'calendar') {
    const cards = ys.months.map((mon) => {
      ...
      const cov = mon.totalWorkdays ? Math.round((mon.coveredWorkdays / mon.totalWorkdays) * 100) : 0;
