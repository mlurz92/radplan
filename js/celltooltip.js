...
function buildHtml(emp, day) {
  ...
  if (meta.position && meta.position !== "—")
    html += `<span class="cdt-pos">${esc(meta.position)}</span>`;
  html += `</div>`;
  ...
  if (hist.length) {
    const items = hist
      .map(
        (h) =>
          `<span class="cdt-hist-item cdt-duty-${h.duty}">${h.duty} ${h.d}.${MONTHS[h.m].slice(0, 3)}</span>`
      )
      .join("");
  ...
  if (change) {
    const when = new Date(change.ts).toLocaleString("de-DE", {
      hour: "2-digit",
      minute: "2-digit",
      day: "2-digit",
      month: "2-digit"
