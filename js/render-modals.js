...
export function openProfileModal(empName) {
  ...
  if (metaRow) {
    ...
    if (meta.phone)
      metaHtml += `<span class="pm-meta-chip pm-chip-phone">
    ...
    if (meta.deputy)
      metaHtml += `<span class="pm-meta-chip pm-chip-deputy">
  ...
  if (todayStatusEl) {
    if (y === TOD_Y && m === TOD_M) {
      ...
      if (todayHol) {
        ...
        if (todayDuty)
          statusText += ` · ${todayDuty === "D" ? "Bereitschaftsdienst" : "Hintergrunddienst"}`;
  ...
  if (kpiEl) {
    ...
    const covPct =
      requiredWorkdays > 0
        ? Math.min(100, Math.round((s.totalActive / requiredWorkdays) * 100))
        : 0;
    ...
    const trend = (cur, prev) => {
      ...
      if (diff < 0)
        return `<span class="kpi-trend dn">▼ ${Math.abs(diff) > 99 ? ">99" : Math.abs(diff)}</span>`;
    ...
    const kpis = [
      {
        label: "Werktage",
        val: s.totalWorkdays,
        sub: `${s.totalActive} aktiv · ${covPct}%`,
        color: "#1D4ED8",
        pct: covPct,
        trendHtml: trend(s.totalActive, sPrev.totalActive),
        ytd: ys.totals.totalActive
      },
      {
        label: "Nicht geplant",
        val: s.uncovered,
        sub: s.uncovered > 0 ? "Arbeitstage offen" : "Vollständig geplant",
        color: s.uncovered > 0 ? "#F97316" : "#15803D",
        pct: 0,
        trendHtml: "",
        ytd: null
      },
      {
        label: "D-Dienste",
        val: s.dutyD.length,
        sub: s.dutyD.length ? s.dutyD.map((d) => `${d}.`).join(" ") : "Keine",
        color: "#EF4444",
        pct: 0,
        trendHtml: trend(s.dutyD.length, sPrev.dutyD.length),
        ytd: ys.totals.dutyD
      },
      {
        label: "HG-Dienste",
        val: s.dutyHG.length,
        sub: s.dutyHG.length ? s.dutyHG.map((d) => `${d}.`).join(" ") : "Keine",
        color: "#0369A1",
        pct: 0,
        trendHtml: trend(s.dutyHG.length, sPrev.dutyHG.length),
        ytd: ys.totals.dutyHG
      },
      {
        label: "Urlaub",
        val: vac,
        ...
        color: "#7C3AED",
        pct: 0,
        trendHtml: trend(vac, vacPrev),
        ytd: ys.totals.vacationDays
      },
      {
        label: "Krank",
        val: sick,
        sub: sick > 0 ? "K · KK" : "Kein Krankentag",
        color: sick > 0 ? "#DC2626" : "#15803D",
        pct: 0,
        trendHtml: trend(sick, sickPrev),
        ytd: ys.totals.sickDays
      },
      {
        label: "FZA",
        val: fza,
        sub: "Freizeitausgleich",
        color: "#3730A3",
        pct: 0,
        trendHtml: "",
        ytd: ys.totals.fzaDays
      },
      {
        label: "Weiterbildung",
        val: wb,
            ...
            options: {
              ...
              plugins: {
                ...
                tooltip: {
                  callbacks: {
                    label: (ctx) =>
                      ` ${ctx.label}: ${ctx.raw} (${Math.round((ctx.raw / totalWP) * 100)}%)`
                  }
                }
              },
  ...
  if (stChartEl) {
    const stEntries = Object.entries(s.stCounts)
      .filter(([, v]) => v > 0)
      .sort((a, b) => b[1] - a[1]);
    if (stEntries.length) {
  ...
  if (dutyDetailEl) {
    if (s.dutyD.length || s.dutyHG.length) {
      ...
      if (s.dutyD.length) {
        const dayBadges = s.dutyD
          .map((d) => {
            const wd = weekday(y, m, d);
            const hol = isHoliday(y, m, d, hols);
            const isWeOrHol = wd === 5 || wd === 6 || wd === 0 || hol;
            const sty = isWeOrHol
              ? ` style="background:#FEF3C7;color:#78350F;border-color:#FDE68A"`
              : "";
        ...
        datasets: [
          {
            label: "Aktive Tage",
            data: activeData,
            backgroundColor: ys.months.map((mon) =>
              mon.m === m ? "rgba(14,165,233,0.85)" : "rgba(14,165,233,0.35)"
            ),
  ...
  if (dowEl) {
    ...
    for (let mon = 0; mon < 12; mon++) {
      ...
      for (let d = 1; d <= dim; d++) {
        ...
        if (
          cell.assignment &&
          WORKPLACES.find((w) => w.code === cell.assignment.split("/")[0].trim())
        ) {
          dowWork[wd]++;
        }
      }
  ...
  if (yrEl) {
    ...
    const totalReqWd = Math.max(
      0,
      ys.totals.totalWorkdays -
    ...
    const totalCov =
      totalReqWd > 0 ? Math.min(100, Math.round((ys.totals.totalActive / totalReqWd) * 100)) : 0;
...
export function hideOverlay(id) {
  ...
  if (el._triggerElement && typeof el._triggerElement.focus === "function") {
...
export function openScoreInfoModal(resultData = autoPlanResult) {
  ...
  const q = {
    ...
    dutyGaps:
      Number(
        resultData?.summary?.quality?.dutyCoverageMisses ?? resultData?.quality?.dutyCoverageMisses
      ) || 0,
    hgGaps:
      Number(
        resultData?.summary?.quality?.hgCoverageMisses ?? resultData?.quality?.hgCoverageMisses
      ) || 0,
    ...
    weSpread:
      Number(resultData?.summary?.quality?.weekendSpread ?? resultData?.quality?.weekendSpread) ||
      0,
    wishes:
      Number(
        resultData?.summary?.quality?.wishFulfillmentRate ??
          resultData?.quality?.wishFulfillmentRate
      ) || 0,
    deepMoves:
      Number(resultData?.summary?.quality?.deepMoves ?? resultData?.quality?.deepMoves) || 0
  };
  ...
  const metrics = [
    {
      label: "D-Abdeckung",
      val: q.dutyGaps === 0 ? "100%" : `${q.dutyGaps} Lücken`,
      weight: "D-Prio",
      hint: "Jede Lücke im Bereitschaftsdienst führt zu massiven Penalty-Abzügen (-15 Punkte pro fehlendem Dienst).",
      pct: Math.max(0, 100 - q.dutyGaps * 20),
      color: q.dutyGaps === 0 ? "#22C55E" : "#EF4444"
    },
    {
      label: "HG-Abdeckung",
      val: q.hgGaps === 0 ? "100%" : `${q.hgGaps} Lücken`,
      weight: "HG-Prio",
      hint: "Jede Lücke im Hintergrunddienst bestraft den Score (-10 Punkte pro fehlendem Dienst).",
      pct: Math.max(0, 100 - q.hgGaps * 20),
      color: q.hgGaps === 0 ? "#22C55E" : "#EF4444"
    },
    {
      label: "BD-Gerechtigkeit",
      val: `Δ ${q.bdSpread}`,
      weight: "Spread",
      hint: "Unterschied zwischen der Person mit den meisten und wenigsten Bereitschaftsdiensten. Exponentieller Abzug ab Δ > 1.",
      pct: Math.max(0, 100 - q.bdSpread * 15),
      color: q.bdSpread <= 1 ? "#22C55E" : "#F59E0B"
    },
    {
      label: "HG-Balance",
      val: `Δ ${q.hgSpread}`,
      weight: "Spread",
      hint: "Gleichmäßige Verteilung im Hintergrunddienst. Strafen skalieren mit zunehmender Ungerechtigkeit.",
      pct: Math.max(0, 100 - q.hgSpread * 20),
      color: q.hgSpread <= 1 ? "#22C55E" : "#F59E0B"
    },
    {
      label: "WE-Streuung",
      val: `Δ ${q.weSpread}`,
      weight: "Spread",
      hint: "Fairness der Wochenend- und Feiertagsdienste. Diese Dienste sind hoch gewichtet und müssen fair rotieren.",
      pct: Math.max(0, 100 - q.weSpread * 25),
      color: q.weSpread <= 1 ? "#22C55E" : "#F59E0B"
    },
    {
      label: "Wunscherfüllung",
      val: `${Math.round(q.wishes * 100)}%`,
      weight: "Bonus",
      hint: "Erfolgsrate der eingetragenen BD/HG-Wünsche. Erfüllte Wünsche generieren Bonuspunkte (bis zu +5.0 auf den Score).",
      pct: Math.round(q.wishes * 100),
      color: q.wishes >= 0.8 ? "#22C55E" : "#93C5FD"
    }
  ];
...
export function showToast(msg, type) {
  ...
  if (!type) {
    type = /fehler|fehlgeschlagen|konnte nicht|ungültig|nicht möglich/i.test(msg)
      ? "error"
      : "success";
  }
...
document.addEventListener("keydown", (e) => {
  ...
  const activeOverlay = visibleOverlays.at(-1);
  const modal = activeOverlay.querySelector(".modal") || activeOverlay;
