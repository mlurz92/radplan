...
import { MONTHS, MONTHS_SHORT, monthKey, isFacharzt, getEmpMeta, posColor } from "./constants.js";
...
function _buildYearData(year) {
  ...
  allEmps.forEach((emp, idx) => {
    ...
    for (let m = 0; m < 12; m++) {
      ...
      const inData = !!(
        DATA[k] &&
        Array.isArray(DATA[k].employees) &&
        DATA[k].employees.includes(emp)
      );
...
function _renderGrid(year, container) {
  ...
  if (!employees.length) {
    container.innerHTML =
      '<div class="yp-empty">Keine Mitarbeitendedaten für dieses Jahr vorhanden.</div>';
    return;
  }
  ...
  ordered.forEach((emp) => {
    ...
    const cells = d.months
      .map((mon, m) => {
        ...
        const heat = d.isDutyCapable
          ? _heat(mon.bd - meansBD[m])
        ...
        const title =
          `${emp} · ${MONTHS[m]} ${year}: ${mon.bd}× D${d.isFa ? ", " + mon.hg + "× HG" : ""}`.replace(
            /"/g,
            "&quot;"
          );
    ...
    const totalBd = d.isDutyCapable
      ? `<span class="yp-total-bd">${d.totalBD}<span class="yp-total-lbl">D</span></span>`
      : '<span class="yp-dash">—</span>';
    const totalHg =
      d.isFa && d.totalHG > 0
        ? `<span class="yp-total-hg">${d.totalHG}<span class="yp-total-lbl">H</span></span>`
        : "";
    ...
    cell.addEventListener("click", () => {
      ...
      if (Number.isFinite(m)) {
        window.dispatchEvent(
          new CustomEvent("radplan-navigate", { detail: { year: ypYear, month: m } })
        );
      }
    });
  });
        ...
        <tbody>
          ${relEmps
            .map((emp) => {
              ...
              const totalMean = means.reduce(
                (sum, mVal, mIdx) => sum + (d.months[mIdx].hasData ? mVal : 0),
                0
              );
...
function _renderProjection(year, container) {
  ...
  if (!dutyEmps.length) {
    container.innerHTML =
      '<div class="yp-empty">Keine dienstfähigen Mitarbeitenden gefunden.</div>';
    return;
  }
  ...
  const projs = dutyEmps.map((emp) => {
    ...
    const progressPct =
      annualTarget > 0 ? Math.min(100, Math.round((d.totalBD / annualTarget) * 100)) : 0;
        ...
        <tbody>
          ${sorted
            .map((p) => {
              ...
              const progCol =
                p.progressPct >= 85 ? "#22C55E" : p.progressPct >= 55 ? "#F59E0B" : "#EF4444";
...
function _posRank(p) {
  const i = _POS_ORDER.indexOf(p);
  return i === -1 ? _POS_ORDER.length : i;
}
...
function _evalToolbar(sortOptions) {
  ...
  const opts = sortOptions
    .map(([v, lbl]) => `<option value="${v}"${_evalSort === v ? " selected" : ""}>${lbl}</option>`)
    .join("");
  return `
  ...
  return employees
    ...
    .map((emp) => {
      ...
      return {
        emp,
        meta,
        perMonth: d.monthlyTarget,
        months: d.monthsWithData,
        soll,
        ist,
        diff,
        quote
...
function _sortEvalRows(rows, keyMap) {
  ...
  rows.sort((a, b) => {
    ...
    if (k === "position") {
      const d = _posRank(a.meta.position) - _posRank(b.meta.position);
...
function _renderSollIst(year, container) {
  ...
  const body = rows
    .map((r) => {
      ...
      const qCol =
        r.quote >= 95 && r.quote <= 110 ? "#15803D" : r.quote > 110 ? "#C2410C" : "#0369A1";
      return `<tr>
...
function _renderAbsence(year, container) {
  ...
  const body = rows
    .map((r) => {
      const pc = posColor(r.meta.position);
      const cell = (v, col) =>
        `<td class="yp-ev-num"${v ? ` style="color:${col};font-weight:700"` : ""}>${v || '<span class="yp-dash">—</span>'}</td>`;
      return `<tr>
      <td class="yp-ev-name" style="border-left:3px solid ${pc.border}">
        <span class="yp-ev-empname">${r.emp}</span>
        <span class="yp-ev-pos" style="color:${pc.fg};background:${pc.bg}">${r.meta.position}</span>
  ...
  container.innerHTML = `
    ${_evalToolbar([
      ["name", "Name (A–Z)"],
      ["position", "Position"],
      ["vacation", "Urlaub (absteigend)"],
      ["sick", "Krank (absteigend)"],
      ["sum", "Σ Abwesenheit"]
    ])}
  ...
  container.querySelector(".yp-eval-export")?.addEventListener("click", () => {
    _downloadCSV(
      `radplan_abwesenheiten_${year}.csv`,
      [
        "Kürzel",
        "Position",
        "Urlaub",
...
export function setupYearPlanModal() {
  ...
  document
    .getElementById("yp-prev-year")
    ?.addEventListener("click", () => setYearPlanYear(ypYear - 1));
  document
    .getElementById("yp-next-year")
    ?.addEventListener("click", () => setYearPlanYear(ypYear + 1));
}
