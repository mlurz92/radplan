...
function doBrowserPrint() {
  ...
  if (footEl)
    footEl.textContent = `RadPlan · Klinik für Radiologie & Nuklearmedizin · ${periodLabel()}`;
...
function buildAutoTableConfig(grid, fontSize, geom, generatedAt, doc) {
  ...
  return {
    ...
    headStyles: {
      fillColor: PRINT_COLORS.headFill,
...
function drawPageChrome(doc, geom, generatedAt, data) {
  ...
  doc.text(
    `RadPlan · Klinik für Radiologie & Nuklearmedizin · erstellt am ${generatedAt}`,
    8,
    geom.pageH - 5
  );
  doc.text(`Seite ${data.pageNumber} / ${totalPages}`, geom.pageW - 8, geom.pageH - 5, {
    align: "right"
  });
}
...
async function doPdfExport() {
  ...
  function measuredHeight(fontSize) {
    ...
    tmp.autoTable(cfg);
    return {
      height: tmp.lastAutoTable.finalY - PDF_LAYOUT.top,
      pages: tmp.internal.getNumberOfPages()
    };
  }
...
function buildModal() {
  ...
  overlay
    .querySelectorAll("[data-pp-close]")
    .forEach((b) => b.addEventListener("click", closePreview));
