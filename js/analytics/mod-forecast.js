...
function seasonalRiskSection(fc) {
  ...
  fc.seasonalIndex.forEach((s) => {
    ...
    const elevated = s.hasData && s.indexVsAverage >= getThresholds().seasonalRiskFactor;
...
function dualBar(r, max) {
  ...
  return `<div class="fc-bars" title="Ist BD ${fmt.int(r.bd)} · Prognose BD ${fmt.int(r.projBd)} · Ziel ${fmt.int(r.yearTarget)}">`
