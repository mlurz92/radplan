...
export function buildProfileStats(y, m, emp) {
  ...
  for (let d = 1; d <= dim; d++) {
    ...
    if ((cell.assignment || cell.duty) && work) {
      coveredWorkdays++;
    }
...
export function buildYearlyStats(emp, year) {
  ...
  for (let m = 0; m < 12; m++) {
    ...
    for (let d = 1; d <= dim; d++) {
      ...
      if ((cell.assignment || cell.duty) && wdDay) {
        cov++;
      }
...
export function getEmployeeYearCardMetrics(emp, year) {
  ...
  const coverage = ys.totals.totalWorkdays > 0 
    ? Math.round((ys.totals.coveredWorkdays / ys.totals.totalWorkdays) * 100) 
    : 0;
