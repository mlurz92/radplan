...
export class NeuralGraph {
  ...
  getAbbreviation(empId) {
    ...
    const stripped = String(empId)
      .replace(
        /^(Herr|Frau|Hr\.|Fr\.|Dr\.\s*(med\.\s*)?|Prof\.\s*(Dr\.\s*(med\.\s*)?|med\.\s*)?|PD\s+Dr\.\s*(med\.\s*)?|Dipl\.\s*\w+\.\s*)/gi,
        ""
      )
      .trim();
  ...
  pulseCell(dayIdx, empId, isActive, isError = false, dutyType = "D") {
    ...
    if (isActive) {
      ...
      if (dutyType === "HG") {
        targetEmp.style.color = targetSlot.classList.contains("has-val")
          ? "#0EA5E9"
          : "transparent";
      } else {
        targetEmp.style.color = targetSlot.classList.contains("has-val")
  ...
  triggerSuccess(finalAssignments) {
    ...
    for (const [dayIdx, cellData] of this.cells.entries()) {
      if (
        cellData.dSlot.classList.contains("has-val") ||
        cellData.hgSlot.classList.contains("has-val")
      ) {
        setTimeout(() => {
  ...
  renderMiniMap() {
    ...
    for (let i = this.pulses.length - 1; i >= 0; i--) {
      ...
      const x = p.direction === 1 ? padX + lineLen * p.progress : w - padX - lineLen * p.progress;
