...
export function renderEmployeeDashboard() {
  ...
  const dutyCount = metrics.reduce(
    (sum, item) => sum + item.ys.totals.dutyD + item.ys.totals.dutyHG,
    0
  );

  const roles = metrics.reduce(
    (acc, item) => {
  ...
  const kpiItems = [
    {
      ...
      sub: `${activeCount} mit Aktivität`,
      tone: "#0EA5E9"
    },
    {
      label: "Aktueller Monatsbestand",
      value: currentMonthData.employees.length,
      sub: `${MONTHS[m]} ${y}`,
      tone: "#22C55E"
    },
    ...
    {
      label: "Rollenmix",
      value: `${roles.lead}/${roles.fa}/${roles.aa}`,
      sub: "Leitung · FA · AA",
      tone: "#A855F7"
    }
  ];

  summaryEl.innerHTML = kpiItems
    .map(
      (item) => `
  ...
  if (filtered.length === 0) {
    ...
    gridEl.innerHTML = filtered
      .map((item) => {
        ...
        // Coverage bar
        const covColor =
          item.coverage >= 80 ? "#22C55E" : item.coverage >= 60 ? "#F59E0B" : "#EF4444";
  ...
  if (detailSub) {
    const viewName =
      dash.detailView === "months"
        ? "Monatsverlauf"
        : dash.detailView === "calendar"
          ? "Jahreskalender"
          : "Verwaltung";
...
function renderEmployeeTeamAnalytics(teamPanelEl, teamControlsEl) {
  ...
  allEmployees.forEach((emp) =>
    perEmp.set(emp, { emp, active: 0, d: 0, hg: 0, vac: 0, sick: 0, uncovered: 0, required: 0 })
  );
  ...
  const rows = [...perEmp.values()].filter(
    (x) => x.active || x.d || x.hg || x.vac || x.sick || x.required
  );
        ...
        <tbody>
          ${topRows
            .map((row) => {
              ...
              const covCls =
                cov >= 80 ? "dept-cov-good" : cov >= 60 ? "dept-cov-mid" : "dept-cov-low";
              return `
...
export function renderEmployeeDetailDashboard(emp, year) {
  ...
  if (state.employeeDashboard.detailView === "months") {
    ...
    const totalCov =
      reqWdTotal > 0 ? Math.min(100, Math.round((ys.totals.totalActive / reqWdTotal) * 100)) : 0;
    ...
    detailEl
      ...
      ?.addEventListener("click", () => openProfileModal(emp));
    return;
  }

  // ===== CALENDAR TAB =====
  if (state.employeeDashboard.detailView === "calendar") {
    const cards = ys.months
      .map((mon) => {
        const vac = VACATION_CODES.reduce((sum, c) => sum + (mon.stCounts[c] || 0), 0);
        const sick = (mon.stCounts["K"] || 0) + (mon.stCounts["KK"] || 0);
        const fza = mon.stCounts["FZA"] || 0;
        ...
        if (mon.dutyD)
          abItems.push(
            `<span class="empdash-mini-chip duty">D <strong>${mon.dutyD}</strong></span>`
          );
        if (mon.dutyHG)
          abItems.push(
            `<span class="empdash-mini-chip hg">HG <strong>${mon.dutyHG}</strong></span>`
          );
        if (vac)
          abItems.push(`<span class="empdash-mini-chip vac">U <strong>${vac}</strong></span>`);
        if (sick)
          abItems.push(`<span class="empdash-mini-chip sick">K <strong>${sick}</strong></span>`);
        if (fza)
          abItems.push(`<span class="empdash-mini-chip fza">FZA <strong>${fza}</strong></span>`);
  ...
  if (state.employeeDashboard.detailView === "analyse") {
    _destroyDetailChart("bar");
    _destroyDetailChart("pie");
        ...
        <div class="empdash-analyse-charts">
          ...
          <div class="empdash-chart-card">
            ...
            <div class="empdash-chart-body empdash-chart-pie-wrap">
              ${
                topWP.length
                  ? `<canvas id="empdash-pie-canvas" aria-hidden="true"></canvas>
              <div class="empdash-pie-legend">
                ${topWP
                  ...
                  : '<div class="empdash-mini-empty" style="padding:24px">Keine Arbeitsplatzdaten</div>'
              }
            </div>
          </div>
        </div>
          ...
          datasets: [
            {
              ...
              backgroundColor: ys.months.map((mon) =>
                mon.m === state.month ? "rgba(14,165,233,0.85)" : "rgba(14,165,233,0.35)"
              ),
              borderRadius: 3,
              borderSkipped: false
            },
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: (ctx) =>
                  ` ${ctx.label}: ${ctx.raw}d (${totalWPDays > 0 ? Math.round((ctx.raw / totalWPDays) * 100) : 0}%)`
              }
            }
          },
